...
export default function WhatsAppFloat() {
  ...
  useEffect(() => {
    try {
      ...
      setConsentResolved(stored === "accepted" || stored === "rejected");
    } catch {
      setConsentResolved(true);
    }
    const handler = () => setConsentResolved(true);
    window.addEventListener("oj-consent-resolved", handler);
    return () => window.removeEventListener("oj-consent-resolved", handler);
  }, []);

  return (
    <a
      href={`https://wa.me/59892001372?text=${encodeURIComponent("Quiero conocer sobre OMODA JAECOO")}`}
      target="_blank"
      ...
      style={{
        bottom: consentResolved
          ...
          : "max(9rem, calc(env(safe-area-inset-bottom) + 8rem))",
